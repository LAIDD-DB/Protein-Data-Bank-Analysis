...
### example:
filter_pdb_list.py -r 205-495 P36897_pdb_list.txt > P36897_pdb_list_filter.txt
...
### example:
./rot.sh list_chain.txt chain/3HMMA.pdb
...
#find_consensus_water.py select/3HMMA_HOH.pdb consensus_water_list.txt
...
### example:
fix_protein.py -i select/3HMMA_receptor_HOH.pdb -o fix/3HMMA_receptor_HOH.pdb

### pdb2pdbqt
### option -r: receptor, -l: ligand
### pdb2pdbqt -i $input_pdb -o $output_pdbqt -r (or -l)
### example
pdb2pdbqt.py -i fix/3HMMA_receptor_HOH.pdb -o fix/3HMMA_receptor_HOH.pdbqt -r
pdb2pdbqt.py -i fix/3HMMA_855.pdb -o fix/3HMMA_855.pdbqt -l

### generate docking box parameters for Vina
./auto_box.sh list_final.txt > config.txt

#cd fix
#qvina02 --config config.txt --ligand 3HMMA_855.pdbqt --out a.pdbqt
#pdbqt2pdb_ref.py -i a.pdbqt -o a.pdb -r 3HMMA_855.pdb
